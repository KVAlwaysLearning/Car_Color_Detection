...
# --- 2. HELPER FUNCTIONS (Exact same logic) ---
...
# --- 3. CORE PROCESSING (Integrated Flow) ---
def process_image(uploaded_file, debug_mode=False):
    ...
    whole_res = models["car_expert"].predict(img_rgb, imgsz=640, conf=0.25, classes=car_classes, augment=True, verbose=False)[0]
    saved_cars = []
    ...
    # --- STEP 2: SIGNAL DETECTION (Multi-Mode Fallback)[cite: 7] ---
    color_modes = get_color_modes(img_bgr)
    modes = list(color_modes.values())
    coords_signals = []
    for mode in modes:
        res = models["yolo26x"].predict(mode, imgsz=1280, conf=0.05, classes=[9], verbose=False)[0]
        for box in res.boxes.xyxy.cpu().numpy():
            if not is_duplicate(box, coords_signals): coords_signals.append(box.tolist())

    # --- STEP 3: PEOPLE ENSEMBLE (Conditional & NMS)[cite: 7] ---
    ...
    if coords_signals:
        ...
        for name, model in models.items():
            ...
            for img_data in modes:
                res = model.predict(img_data, imgsz=1280, conf=0.30, verbose=False)[0]
    ...
    if debug_mode:
        debug_logs.append(f"Res: {w}x{h} | Total Cars: {final_car_count} | Signals: {len(coords_signals)}")
...
with st.sidebar:
    ...
    for m in models.keys(): st.write(f"✅ {m}")
...
if uploaded_file:
    ...
    with col1: st.image(cv2.cvtColor(res_img, cv2.COLOR_BGR2RGB), use_container_width=True)
    with col2:
