...
def is_blue_car_robust(car_crop_rgb):
    ...
    # Strict mask logic from Integrated codes.txt
...
def process_image(uploaded_file):
    ...
    # --- DYNAMIC ID DISCOVERY (STRICT FILTERING) ---
    def get_ids(model, keywords):
        return [id for id, name in model.names.items() if any(k == name.lower() for k in keywords)]

    ids = {
        name: {
            "person": [id for id, n in mod.names.items() if any(k in n.lower() for k in ["person", "pedestrian", "rider"])],
            "car": get_ids(mod, ["car"]), # Strictly "car" only
            "signal": get_ids(mod, ["traffic light"]) # Strictly "traffic light" only
    ...
    for box in saved_cars:
        ...
        if is_blue_car_robust(img_rgb[y1:y2, x1:x2]) > 0.30: # Restored 0.30 threshold
            blue_count += 1
            cv2.rectangle(display_img, (x1, y1), (x2, y2), (0, 0, 255), 3) # Red for Blue Car
        else:
            cv2.rectangle(display_img, (x1, y1), (x2, y2), (255, 0, 0), 3) # Blue for Normal Car

    # --- STEP 2: SIGNAL DETECTION (TIERed SCAN) ---
    unique_signals = []
    sig_ids = ids["yolo26x"]["signal"]
    
    # Tier 1: Full Pass (3 Modes)
    ...
    # Tier 2: Deep Strip Scan (If Tier 1 Fails)
    if not unique_signals:
        ...
        for i in range(10):
            for m_name in ["RGB", "BGR", "Grey"]:
                ...
                sh, sw = strip_h.shape[:2]
                res = models["yolo26x"].predict(cv2.resize(strip_h, (640, 640)), conf=0.05, classes=sig_ids, verbose=False)[0]
                for b in res.boxes.xyxy.cpu().numpy():
                    g_box = [b[0]*(sw/640), b[1]*(sh/640)+h_steps[i], b[2]*(sw/640), b[3]*(sh/640)+h_steps[i]]
                    if not is_duplicate(g_box, unique_signals): unique_signals.append(g_box)
                # Vertical
                strip_v = modes_dict[m_name][0:h, w_steps[i]:w_steps[i+1]]
                sh, sw = strip_v.shape[:2]
                res = models["yolo26x"].predict(cv2.resize(strip_v, (640, 640)), conf=0.05, classes=sig_ids, verbose=False)[0]
                for b in res.boxes.xyxy.cpu().numpy():
                    g_box = [b[0]*(sw/640)+w_steps[i], b[1]*(sh/640), b[2]*(sw/640)+w_steps[i], b[3]*(sh/640)]
                    if not is_duplicate(g_box, unique_signals): unique_signals.append(g_box)

    # --- STEP 3: PEOPLE ENSEMBLE (CONDITIONAL) ---
    p_count = 0
    scene = "Traffic Signal Scene" if unique_signals else "Normal Scene"
    
    if unique_signals:
        all_p_boxes, all_p_confs = [], []
        # 15-Pass Ensemble (5 models x 3 modes)
